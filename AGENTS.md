...
- 严禁虚报、夸大或提前宣称实现/验证进度；只能基于已经完成的代码和实际运行结果汇报。实现时必须避免过度设计，优先满足当前明确验收标准；
- 必须遵守已经写入 `DESIGN.md`、`TODO.md` 和相关任务文档的设计与文档约定。进入一个实现迭代后，不得在中途改变目标、验收标准或架构方向来迎合临时代码；如果确需调整设计，必须先结束当前迭代或在新迭代开始前更新相关文档，再按更新后的设计实现；
- 与当前迭代相关的文档只能在迭代开始前或迭代结束后修改。实现过程中发现的设计问题应先记录为 blocker 或下一迭代事项，不能边写代码边漂移设计；
