...
class MuJoCoBackend(SimulatorBackend):
    ...
    def servo_control_stream(
        ...
        for request in request_iterator:
            if request.HasField("joint_cmd"):
                ...
                # convert velocities in world coordinate system (Cartesian) to velocities in joint coordinate system.
    ...
    ########## methods (below) that do NOT need to be implemented for now ##########
    ...
    def _solve_end_effector_twist(
        ...
        # to avoid numerical issues caused by singular Jacobian matrices (such as singular configurations),
        # the DLS (Damped Least Squares) method is used.
        # NOTE: A fixed damping value of {@code EE_DLS_DAMPING} introduces unnecessary tracking errors in non-singular configurations.
        # TODO: The damping coefficient can be adaptively adjusted based on the singular values of the Jacobian matrix (e.g., `np.linalg.svd(jacobian)`),
        # increasing damping near singular configurations and decreasing it far from them.
