...
`vsim` 时已经不是 distribution，而是一个具体 task instance 的固定定义。CSD 的
唯一 canonical persistence 是 composed OpenUSD stage；不再保留等价 JSON CSD。
stage 的 root layer 固定为 `csd/<csd_id>/csd.usda`，并 composition scenario、task、
portable physics 与 backend physics layers。JSON 继续用于 manifest、diagnostics、
asset registry 与 trace，但不能成为第二份 CSD 语义源。

CSD stage 优先使用标准 `UsdGeom`、`UsdLux`、`UsdShade` 与 `UsdPhysics` schemas。
只有 task/evaluator、provenance、sampled randomization、runtime sensor requirements
与 typed semantic relationships 等标准 schema 未覆盖的语义，才使用 project
namespace 下的 codeless schemas。`/World` 上的 `physicsBackend` variant set 选择
`mujoco`、`pybullet` 或 `gazebo` opinions；MuJoCo 扩展优先使用官方
`mjcPhysics`，其余 backend extension 必须 project-namespaced。backend variant
不得改写 task/entity identity 或 sampled values。
...
- PyBullet Quickstart Guide / Bullet documentation:
...
- OpenUSD asset resolution and composition references:
  ...
  `https://openusd.org/release/api/_usd__page__generating_schemas.html`,
...
- MuJoCo OpenUSD import/build and `mjcPhysics`:
...
该布局消费真实的 composed OpenUSD CSD，而不是在 JSON 中模仿 USD。CSD stage
持有 environment、robot、objects、enum relationships、材质、collision 与已采样
override；asset registry 持有 project-owned asset provenance 与 backend resource
adapters。backend native loader/compiler 负责选择 `physicsBackend` variant，并把
依赖解析或复制为 realization package 内的相对路径。

所有 accepted CSD fixtures 必须满足：OpenUSD stage 可 composition open；
`usdchecker --strict` 通过；所有 sublayer/reference/payload/mesh/texture path 可解析；
project semantic validator 能确认 required prims、schemas、entity refs、relationship
...
`initial_state.contact` 当前只支持与 MuJoCo `geom` 直接对应的字段：
`margin_m` -> `margin`、`gap_m` -> `gap`、`solref`、`solimp`。这些字段只写入
承载 collision/mass/friction 的 geom；若使用单一 visual/collision geom，则写入该
geom，若提供独立 `collision_mesh_path`，则写入透明 collision geom。compiler 会在
写出前阻止非正 mass、负 friction、负 contact margin/gap，以及 `gap_m > margin_m`
的 CSD。
...
box surface 的 loaded `size`、`friction` 和 `rgba`；若该检查失败，compiler 返回
...
artifact。全部 realization diagnostics 通过后，compiler 会写入
`diagnostics/validation_record.json`，以 typed `CsdRealizationValidationRecord`
汇总 CSD ID、backend manifest ID、cache key、diagnostic evidence files 和 preview
files；若 relationship/physics check、渲染或输出为空失败，同样返回 blocker。
...
`diagnostics/semantic_preview.ppm`、`diagnostics/validation_record.json`，并把临时
Franka robot template 与其 mesh dependency closure 复制到当前 realization
package。这样即使原始 asset cache 或
...
visual/collision geometry, unsupported-feature blockers, and all three backend
variants. At least one MuJoCo smoke fixture must render a nonblank image and
assert compiled entity, transform, mass/inertia, collision, joint, gravity, and
...
Anonymous `dict[str, Any]` access and a generated JSON sidecar are prohibited.
...
└─────────────────────────────────────────────────────────────┘
          │
┌─────────▼───────────────────────────────────────────────────┐
│              PyBulletBackend (Concrete)                     │
│  - 使用 PyBullet DIRECT/GUI physics client                    │
│  - 通过 URDF + package-local assets 加载机器人和刚体对象        │
│  - 由编译器生成的 Python loader 组装 scene、surface、dynamics   │
│  - scene metadata 记录 cameras/sensors/entity mappings        │
│  - 后台线程自动 stepSimulation，支持 joint/ee servo 与录制回放   │
└─────────────────────────────────────────────────────────────┘
```
...
| 能力 | 检测方式 | 说明 |
|------|----------|------|
| JOINT_READ | robot body loaded 且存在 revolute/prismatic joint | 通过 `getJointInfo` / `getJointState` 读取 |
| JOINT_WRITE | 存在可控 revolute/prismatic joint | 使用 `setJointMotorControlArray` |
| END_EFFECTOR_READ | SRDF 或 fallback tip link 可解析 | 通过 `getLinkState` 做 FK |
| NAVIGATION | 不可用 | PyBullet runtime 不伪造 Nav2/SLAM 能力 |
| SENSOR_CAMERA | realization metadata 或默认 scene 声明 camera | 使用 `getCameraImage` 渲染 |
| SENSOR_JOINT | 始终注册 `joint_states` 伪传感器 | 与 MuJoCo recorder/policy 语义一致 |
| SIMULATION_CONTROL | 可用 | backend 维护自动步进，也支持 reset/pause/resume/step |
...
### 1. Realization package

PyBullet 的 CSD realization 不把单个 URDF 当作完整 scene。完整后端产物是
`engine_manifests/pybullet/<csd_id>/` 下的自包含 package：
...
URDF 只负责表达物理 body、link、joint、visual/collision/inertial 等结构；
compiler 生成的 `scene.py` 负责以 PyBullet API 确定性组装 scene，例如连接后的
search path、gravity、URDF body loading、procedural surface、dynamics/friction、
initial pose 和 reset 语义。`scene_meta.json` 是 auditable metadata，记录 CSD
entity 到 PyBullet body/link 的映射、camera/sensor 声明、preview/validation
期望和后端特定配置。运行时应从 `manifest.json` 所在目录解析相对路径，不信任
可能过期的绝对 `root_path`。

生成的 Python loader 是 compiler-owned artifact，不是开放脚本扩展点：仅允许
依赖 Python 标准库与 `pybullet`，只能访问当前 realization package 内的相对资源，
并暴露稳定入口 `load_scene(physics_client_id: int) -> dict[str, object]`。
后端运行时可以导入该 loader，但语义判断应优先依据 `scene_meta.json`，避免把
任意 Python 代码当作新的语义源。

### 2. CSD -> PyBullet mapping

PyBullet realization 必须像 MuJoCo/Gazebo 一样先经过 asset backend compatibility
gate。即使 PyBullet 可加载 URDF/SDF/MJCF，当前路径也不直接复用 MuJoCo MJCF：
PyBullet 的 MJCF/SDF import 会忽略或弱化 cameras/lights 等 scene 语义，不能作为
本框架的 sensor/validation 来源。对象和机器人应使用 `backend="pybullet"` 的
resource adapter；允许 adapter 指向与其他 backend 相同的 mesh 文件，但必须显式
记录为 PyBullet adapter。

当前 PyBullet compiler 的目标不是宣称已经覆盖 MuJoCo compiler 的全部语义面，
而是在已选定的共享 CSD fixture 上达到同级 realization discipline：对于
Franka tabletop robot-bearing CSD，PyBullet package 必须包含可加载的机器人、
对象、surface、camera/sensor metadata、manifest、diagnostics 和 preview；
runtime 从该 manifest 启动后应暴露与默认 PyBullet Panda 场景一致的 robot
state/spec/control 能力。同一 CSD 的 MuJoCo 与 PyBullet preview 应输出到可比较
...
- package-local URDF robot template。`robot_franka_panda`/`franka_panda` 当前映射到
  PyBullet data 的 Franka Panda URDF，但 compiler 必须把所需 URDF/mesh dependency
  closure 复制到 realization package，而不是在 generated scene 中依赖
  `pybullet_data` search path；
- rigid mesh object，输出 object URDF 或由 loader 创建 `createMultiBody`；
- box surface，优先由 loader 创建 procedural collision/visual shape；
- CSD camera 记录到 `scene_meta.json` 并由 runtime `getCameraImage` 渲染；
- object mass/friction 通过 URDF inertial 与 `changeDynamics` 映射；
- 不存在明确 PyBullet 等价语义的 MuJoCo solver 参数（例如 `solref`/`solimp`）
  必须返回 blocker，不能静默降级。

如果 CSD 声明了 robot，但 PyBullet compiler 没有该 robot 的显式映射或 package-local
资源复制策略，compiler 必须返回 blocker；不得生成缺失 robot 的 object-only scene。

仍未达到 MuJoCo compiler 覆盖面的 PyBullet gaps 必须显式保留为 gaps 或 blockers，
不能写成已完成能力：MuJoCo 当前已有更完整的 relationship validation、contact
margin/gap/solver 参数映射、更多正向/负向 fixture 覆盖和材质/texture 语义测试。
PyBullet 后续补齐这些能力时应逐项增加共享 CSD fixture，并把两个 backend 的产物
写入稳定 comparison output 目录供人工逐项检查。

### 3. Runtime

PyBulletBackend 使用 `p.DIRECT` 或 `p.GUI` 创建独立 physics client，默认关闭
real-time simulation，由后台线程按 fixed timestep 调用 `stepSimulation`。这与
MuJoCoBackend 的自动步进语义一致：用户不需要显式调用 `StepPhysics` 才能运行仿真，
但 PyBullet 后端可以实现 pause/resume/manual step 以服务同步训练场景。

Robot state/spec、LeRobot recorder/replay 和 policy runtime 继续通过
`SimulatorBackend` 抽象复用。joint action 语义保持为 joint-space absolute
position target；velocity/twist/torque 控制下的 `get_joint_command_state()` 必须
返回可 replay 的状态，不能把不可 replay 的原始命令写入 dataset。
