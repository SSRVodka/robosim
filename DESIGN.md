...
当前 PyBullet compiler 的完成目标是达到 MuJoCo compiler 的同级 completeness：
对于同一个 CSD，PyBullet package 必须包含可加载的机器人、对象、surface、
camera/sensor metadata、manifest、diagnostics 和 preview；runtime 从该 manifest
启动后应暴露与默认 PyBullet Panda 场景一致的 robot state/spec/control 能力。
同一 CSD 的 MuJoCo 与 PyBullet preview 应通过视觉检查确认主体布局相似，差异仅能
来自 simulator renderer、材质和 contact/numeric 语义差异。
